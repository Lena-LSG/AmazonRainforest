...
plt.scatter(fdates,fmean, s=500, c='blue') # Plot the data as a line graph.
...
plt.yticks(size=80)
corrcoef, pval = stats.pearsonr(x, fmean)
corrcoef = "{:.3f}".format(corrcoef)
pval = "{:.3f}".format(pval)
plt.text(fdates[3],-0.25, "r = " + str(corrcoef) + ", p = " + str(pval), fontsize = 80)
plt.plot(x, p(x), "b-", linewidth=10)
plt.axhline(y=0, color = "black", linestyle='-', linewidth = 10)
ax.tick_params(axis='both', which='major', labelsize=40)
plt.title("Average-Subtracted Precipitation from Jun 2000 - Sep 2021", size = 100) # Set figure title.
plt.xlabel("Date (YYYY-MM)", size = 100) # Set x-axis label.
plt.ylabel("Average Deviation in Precipitation (mm/hr)", size = 100) # Set y-axis label.
plt.savefig("Precipitation_Anomalies_Corr.png") # Save plot as a png.