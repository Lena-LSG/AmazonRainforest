...
fdata = open('NDVI_outdata.txt', 'r') # Open data file in read-only mode.
...
plt.plot(fdates,fmean,'g-', linewidth=10) # Plot the data as a line graph.
...
plt.yticks(size=80)
plt.text(fdates[3],0.012, "Total NDVI Deviation Change = " + tChange + " +/- " + terror, fontsize = 80)
plt.plot(x, p(x), "g--", linewidth=10)
plt.axhline(y=0, color = "black", linestyle='-', linewidth = 10)
ax.tick_params(axis='both', which='major', labelsize=40)
plt.title("Average-Subtracted NDVI from Jun 2002 - Nov 2021", size = 100) # Set figure title.
plt.xlabel("Date (YYYY-MM)", size = 100) # Set x-axis label.
plt.ylabel("Average Deviation in NDVI", size = 100) # Set y-axis label.
plt.savefig("NDVI_Time_Series_Anomalies.png") # Save plot as a png.