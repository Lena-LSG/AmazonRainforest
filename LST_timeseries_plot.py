...
fdata = open('LST_outdata.txt', 'r') # Open data file in read-only mode.
...
plt.yticks(size=80)
plt.text(fdates[3],303.5, "Total LST Change = " + tChange + " +/- " + terror + "K", fontsize = 80)
plt.plot(x, p(x), "y--", linewidth=10)
ax.tick_params(axis='both', which='major', labelsize=40)
plt.title("Average Daytime LST", size = 100) # Set figure title.
plt.xlabel("Date (YYYY-MM)", size = 100) # Set x-axis label.
plt.ylabel("Average Daytime LST (K)", size = 100) # Set y-axis label.
plt.savefig("LST_Time_Series.png") # Save plot as a png.