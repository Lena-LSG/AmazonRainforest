...
bdata = open('NDVI_outdata.txt', 'r')
blines = bdata.readlines()
bdate = [line.split()[0] for line in blines]
bNDVI = [line.split()[1] for line in blines]
...
bNDVI = np.array(bNDVI)
bNDVI = bNDVI.astype(np.float64)
...
for i in range(len(fdate)):
	...
	if month == 1:
		bjanCount = bjanCount + 1
		bjanAvg = bjanAvg + bNDVI[i]

	elif month == 2:
		bfebCount = bfebCount + 1
		bfebAvg = bfebAvg + bNDVI[i]

	elif month == 3:
		bmarCount = bmarCount + 1
		bmarAvg = bmarAvg + bNDVI[i]

	elif month == 4:
		baprCount = baprCount + 1
		baprAvg = baprAvg + bNDVI[i]

	elif month == 5:
		bmayCount = bmayCount + 1
		bmayAvg = bmayAvg + bNDVI[i]

	elif month == 6:
		bjunCount = bjunCount + 1
		bjunAvg = bjunAvg + bNDVI[i]

	elif month == 7:
		bjulCount = bjulCount + 1
		bjulAvg = bjulAvg + bNDVI[i]

	elif month == 8:
		baugCount = baugCount + 1
		baugAvg = baugAvg + bNDVI[i]

	elif month == 9:
		bsepCount = bsepCount + 1
		bsepAvg = bsepAvg + bNDVI[i]

	elif month == 10:
		boctCount = boctCount + 1
		boctAvg = boctAvg + bNDVI[i]

	elif month == 11:
		bnovCount = bnovCount + 1
		bnovAvg = bnovAvg + bNDVI[i]

	elif month == 12:
		bdecCount = bdecCount + 1
		bdecAvg = bdecAvg + bNDVI[i]


bjanAvg = bjanAvg / bjanCount
bfebAvg = bfebAvg / bfebCount
bmarAvg = bmarAvg / bmarCount
baprAvg = baprAvg / baprCount
bmayAvg = bmayAvg / bmayCount
bjunAvg = bjunAvg / bjunCount
bjulAvg = bjulAvg / bjulCount
baugAvg = baugAvg / baugCount
bsepAvg = bsepAvg / bsepCount
boctAvg = boctAvg / boctCount
bnovAvg = bnovAvg / bnovCount
bdecAvg = bdecAvg / bdecCount

for m in range(len(fdate)):
	l = len(fdate[m])
	month = int(fdate[m][4:])

	if month == 1:
		bNDVI[m] = bNDVI[m] - bjanAvg

	elif month == 2:
		bNDVI[m] = bNDVI[m] - bfebAvg

	elif month == 3:
		bNDVI[m] = bNDVI[m] - bmarAvg

	elif month == 4:
		bNDVI[m] = bNDVI[m] - baprAvg

	elif month == 5:
		bNDVI[m] = bNDVI[m] - bmayAvg

	elif month == 6:
		bNDVI[m] = bNDVI[m] - bjunAvg

	elif month == 7:
		bNDVI[m] = bNDVI[m] - bjulAvg

	elif month == 8:
		bNDVI[m] = bNDVI[m] - baugAvg

	elif month == 9:
		bNDVI[m] = bNDVI[m] - bsepAvg

	elif month == 10:
		bNDVI[m] = bNDVI[m] - boctAvg

	elif month == 11:
		bNDVI[m] = bNDVI[m] - bnovAvg

	elif month == 12:
		bNDVI[m] = bNDVI[m] - bdecAvg
...
z, V = np.ma.polyfit(fmean, bNDVI, 1, cov = True)
p = np.poly1d(z)
...
plt.scatter(fmean,bNDVI, s=500, c='blue') # Plot the data as a line graph.
...
#plt.xlim([fmean[0],fmean[-1]])
plt.xticks(size=80) # Plot x tics using formatted date-time array.
plt.yticks(size=80)
corrcoef, pval = stats.pearsonr(fmean, bNDVI)
corrcoef = "{:.3f}".format(corrcoef)
pval = "{:.3f}".format(pval)
plt.text(-0.15,-0.015, "r = " + str(corrcoef) + ", p = " + str(pval), fontsize = 80)
plt.plot(fmean, p(fmean), "b-", linewidth=10)
plt.axhline(y=0, color = "black", linestyle='-', linewidth = 10)
ax.tick_params(axis='both', which='major', labelsize=40)
plt.title("Correlation of Precipitation and NDVI Deviations from Jun 2002 - Sep 2021", size = 100) # Set figure title.
plt.xlabel("Average Deviation in Precipitation (mm/hr)", size = 100) # Set x-axis label.
plt.ylabel("Average Deviation in NDVI", size = 100) # Set y-axis label.
plt.savefig("NDVI_Precipitation_Anomalies_Corr.png") # Save plot as a png.