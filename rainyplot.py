...
fdata = open('yearly_output.txt', 'r') # Open data file in read-only mode.
...
print("x_2: {} +/- {}".format(z[1], np.sqrt(V[1][1])))

plt.figure(figsize=(100,40))
plt.plot(fdates,fmean,'b-') # Plot the data as a line graph.
...
#plt.gca().xaxis.set_major_locator(mdates.MonthLocator(bymonth=(12,6)))
...
plt.title("Detrended Average Precipitation", size = 50) # Set figure title.
plt.xlabel("Date (YYYY)", size = 50) # Set x-axis label.
plt.ylabel("Average Precipitation Deviation from Trend (mm/hr)", size = 50) # Set y-axis label.
plt.savefig("Detrended_Yearly_Precipitation_Time_Series.png") # Save plot as a png.